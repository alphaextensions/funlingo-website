    ...
    <div className="flex flex-col min-h-screen bg-[var(--bg)]">
      ...
      <section className="relative w-full py-16 lg:py-24 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto flex flex-col gap-6 text-center lg:text-left">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-[var(--text)] leading-tight">What Is Shadowing in Language Learning? (+ AI Tools)</h1>
      ...
      <main className="w-full max-w-3xl mx-auto px-6 pb-24">
        <div className="prose prose-lg max-w-none text-[var(--text-dim)]">
          ...
          <div className="bg-[var(--surface)] border-l-4 border-[#C81FD4] p-6 rounded-r-lg my-8">
            <p className="m-0 text-[var(--text)]"><strong className="text-[var(--text)]">Quick definition:</strong> Language shadowing means listening to native speech and repeating it aloud simultaneously &mdash; matching the speaker&apos;s rhythm, intonation, and pronunciation as closely as possible, typically with a 1-2 second delay.</p>
          </div>

          <hr className="border-[var(--border)] my-10" />

          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">The Science Behind Shadowing</h2>
          ...
          <div className="grid md:grid-cols-3 gap-6 my-8">
            {[
              ...
              <div key={i} className="bg-[var(--surface)] p-6 rounded-xl border border-[var(--border)]">
                <div className="text-[#C81FD4] mb-3">{item.icon}</div>
                <div className="text-[var(--text)] font-bold mb-2">{item.title}</div>
                <p className="text-sm text-[var(--text-dim)] m-0">{item.desc}</p>
              </div>
            ))}
          </div>

          <p>A 2019 study published in <em>Language Learning</em> found that learners who practiced shadowing for just 10 minutes a day over 4 weeks showed statistically significant improvements in pronunciation accuracy and speech fluency compared to a control group that only did listening exercises. The key difference? Shadowing engages your <strong className="text-[var(--text)]">motor cortex</strong> alongside your auditory cortex, creating stronger neural pathways for language production.</p>

          <h2 className="text-3xl font-bold text-[var(--text)] mt-16 mb-6">Step-by-Step: How to Shadow Effectively</h2>
          ...
          <div className="space-y-6 my-8">
            {[
              ...
              <div key={item.step} className="flex gap-4 p-5 rounded-lg bg-[var(--surface)] border border-[var(--border)]">
                <div className="flex-shrink-0 w-10 h-10 rounded-full bg-[#C81FD4]/20 text-[#C81FD4] flex items-center justify-center font-bold text-lg">{item.step}</div>
                <div>
                  <strong className="text-[var(--text)] block mb-1">{item.title}</strong>
                  <span className="text-[var(--text-dim)] text-sm">{item.desc}</span>
                </div>
              </div>
            ))}
          </div>

          <h2 className="text-3xl font-bold text-[var(--text)] mt-16 mb-6">Best Content Sources for Shadowing</h2>
          ...
          <div className="grid sm:grid-cols-2 gap-6 my-8">
            {[
              ...
              <div key={i} className="bg-[var(--surface)] p-6 rounded-xl border border-[var(--border)]">
                <div className="flex items-center gap-2 text-[#C81FD4] mb-3">{item.icon}<span className="text-[var(--text)] font-bold">{item.title}</span></div>
                <p className="text-sm text-[var(--text-dim)] m-0">{item.desc}</p>
              </div>
            ))}
          </div>

          <h2 className="text-3xl font-bold text-[var(--text)] mt-16 mb-6">How AI Tools Enhance Shadowing</h2>
          ...
          <div className="bg-[var(--surface)] rounded-xl p-8 border border-[var(--border)] my-8">
            <h3 className="text-xl font-bold text-[var(--text)] mb-4 mt-0">What AI brings to shadowing</h3>
            <ul className="space-y-3 list-none pl-0 m-0">
              {[
                ...
                <li key={i} className="flex items-start gap-3">
                  ...
                  <span className="text-[var(--text-dim)] text-sm" dangerouslySetInnerHTML={{ __html: item }} />
                </li>
              ))}
            </ul>
          </div>

          <h2 className="text-3xl font-bold text-[var(--text)] mt-16 mb-6">How Funlingo Dual Subtitles Help with Shadowing</h2>
          <p>One of the biggest challenges with shadowing is understanding what you are repeating. Without comprehension, shadowing becomes mindless parroting. This is where <a href="/blog/best-dual-subtitle-extension" className="text-[#C81FD4] hover:underline">dual subtitles</a> make a transformative difference.</p>
          ...
          <div className="bg-[rgba(200,31,212,0.05)] border border-[#C81FD4]/20 rounded-xl p-8 my-8">
            <h3 className="text-xl font-bold text-[var(--text)] mb-4 mt-0">Funlingo + Shadowing workflow</h3>
            <p className="text-[var(--text-dim)] mb-4">With Funlingo installed, your shadowing sessions on Netflix, YouTube, or Prime Video become dramatically more effective:</p>
            <ul className="space-y-3 list-none pl-0 m-0">
              {[
                ...
                <li key={i} className="flex items-start gap-3">
                  <Check className="w-5 h-5 text-[#C81FD4] mt-1 shrink-0" />
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mt-16 mb-6">Common Shadowing Mistakes to Avoid</h2>
          <ul className="space-y-4 list-none pl-0">
            {[
              ...
              <li key={i} className="flex gap-4 p-4 rounded-lg bg-[var(--surface)] border border-[var(--border)] hover:border-[var(--border)] transition-colors">
                ...
                <div><strong className="text-[var(--text)] block mb-1">{item.title}</strong><span className="text-[var(--text-dim)] text-sm">{item.desc}</span></div>
              </li>
            ))}
          </ul>

          <h2 className="text-3xl font-bold text-[var(--text)] mt-16 mb-6">30-Day Shadowing Challenge Plan</h2>
          ...
          <div className="space-y-6 my-8">
            {[
              ...
              <div key={i} className="bg-[var(--surface)] rounded-xl p-6 border border-[var(--border)]">
                <div className="flex items-center gap-3 mb-3">
                  <span className="bg-[#C81FD4]/20 text-[#C81FD4] px-3 py-1 rounded-full text-sm font-bold">{item.week}</span>
                  <span className="text-[var(--text)] font-bold">{item.title}</span>
                </div>
                <p className="text-[var(--text-dim)] text-sm m-0">{item.tasks}</p>
              </div>
            ))}
          </div>
