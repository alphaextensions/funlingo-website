  ...
  return (
    <div className="flex flex-col min-h-screen bg-[var(--bg)]">
      ...
      <section className="relative w-full py-16 lg:py-24 px-6 overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-amber-900/40 via-[var(--bg)] to-[var(--bg)] z-0 pointer-events-none" />
        <div className="relative z-10 max-w-4xl mx-auto flex flex-col gap-6 text-center lg:text-left">
          ...
          <h1 className="font-heading-h1 text-4xl sm:text-5xl lg:text-6xl font-bold text-[var(--text)] leading-tight">
      ...
      <main className="w-full max-w-3xl mx-auto px-6 pb-24">
        <div className="prose prose-invert prose-lg max-w-none text-[var(--text-dim)]">
          ...
          <p className="lead text-xl leading-relaxed">
            The word <strong className="text-[var(--text)]">&ldquo;unless&rdquo;</strong> is one of those small English words that trips up learners at every level. It looks simple, but its logic can be surprisingly tricky. Whether you&apos;re preparing for an English exam, writing a formal email, or trying to follow dialogue in a Netflix series, understanding <strong className="text-[var(--text)]">unless</strong> is essential. In this guide, we cover the full meaning, grammar rules across all conditional types, 30+ example sentences, common mistakes, translations into other languages, and how immersion tools can help you internalize grammar naturally.
          </p>

          <div className="bg-[var(--surface)] border-l-4 border-amber-400 p-6 rounded-r-lg my-8">
            <p className="m-0 text-[var(--text)]">
              <strong className="text-[var(--text)]">Quick definition:</strong> <strong className="text-[var(--text)]">Unless</strong> means <em>&ldquo;except if&rdquo;</em> or <em>&ldquo;if&nbsp;&#8230;&nbsp;not.&rdquo;</em> It introduces the only condition under which a statement would not be true. For example: <em>&ldquo;I won&apos;t leave unless you ask me to&rdquo;</em> = <em>&ldquo;I won&apos;t leave if you don&apos;t ask me to.&rdquo;</em>
            </p>
          </div>
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">What Does &ldquo;Unless&rdquo; Mean?</h2>
          <p>
            <strong className="text-[var(--text)]">Unless</strong> is a subordinating conjunction. It connects a dependent clause (the condition) to a main clause (the result). The core meaning is <em>&ldquo;except under the circumstance that&rdquo;</em> or, more simply, <em>&ldquo;except if.&rdquo;</em>
          </p>
          <p>
            Think of <strong className="text-[var(--text)]">unless</strong> as a negative gate. The action in the main clause will happen (or won&apos;t happen) in every scenario <em>except</em> the one described in the unless-clause. This is why <strong className="text-[var(--text)]">unless</strong> and <strong className="text-[var(--text)]">if&nbsp;&#8230;&nbsp;not</strong> often mean the same thing &mdash; but not always, as we&apos;ll explore below.
          </p>
          <p>
            Here is the simplest way to remember it:
          </p>
          <div className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6 my-6">
            <p className="m-0 text-[var(--text)] text-center text-lg">
              <strong className="text-[var(--text)]">Unless</strong> = <strong className="text-[var(--text)]">If&nbsp;&#8230;&nbsp;not</strong> (in most cases)
            </p>
          </div>
          <p>
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">&ldquo;Unless&rdquo; vs &ldquo;If Not&rdquo; &mdash; When They&apos;re Interchangeable and When They&apos;re Not</h2>
          <p>
            Many textbooks say <strong className="text-[var(--text)]">unless = if not</strong>, and for zero and first conditional sentences that is usually true. However, there are important situations where you <em>cannot</em> swap one for the other.
          </p>

          <h3 className="text-xl font-bold text-[var(--text)] mb-4">When you CAN use either</h3>
          <div className="space-y-3 my-6">
            {[
              ...
              <div key={i} className="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-4">
                <p className="m-0 text-[var(--text)]"><strong className="text-[var(--text)]">Unless:</strong> {item.sentence}</p>
                <p className="m-0 text-[var(--text-dim)] text-sm mt-1"><strong>Equivalent:</strong> {item.alt}</p>
              </div>
            ))}
          </div>

          <h3 className="text-xl font-bold text-[var(--text)] mb-4">When you CANNOT use &ldquo;unless&rdquo;</h3>
          <p>
            <strong className="text-[var(--text)]">Unless</strong> does not work with unreal or hypothetical conditions. It is restricted to <em>open</em> conditions &mdash; situations that are still possible. Here are the key restrictions:
          </p>
          <div className="space-y-3 my-6">
            {[
              ...
              <div key={i} className="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-4">
                ...
                <p className="m-0 text-[var(--text-dim2)] text-sm italic">{item.reason}</p>
              </div>
            ))}
          </div>
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">Grammar Rules for &ldquo;Unless&rdquo;</h2>
          <p>
            Understanding which conditional types work with <strong className="text-[var(--text)]">unless</strong> is the single most important grammar point. Let&apos;s break it down.
          </p>
            ...
            {[
              ...
              <div key={i} className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <h3 className="text-xl font-bold text-[var(--text)] m-0">{item.title}</h3>
                </div>
                ...
                <ul className="space-y-1 m-0 pl-0 list-none mb-3">
                  {item.examples.map((ex, j) => (
                    <li key={j} className="flex items-start gap-2 text-[var(--text-dim)]">
                      <ArrowRight className="w-4 h-4 text-amber-400 mt-1 flex-shrink-0" />
                      <span><em>{ex}</em></span>
                    </li>
                  ))}
                </ul>
                <p className="m-0 text-[var(--text-dim2)] text-sm">{item.note}</p>
              </div>
            ))}
          </div>
          ...
          <h3 className="text-xl font-bold text-[var(--text)] mb-4">Additional Grammar Notes</h3>
          <ul className="space-y-2 my-4">
            <li className="flex items-start gap-2 text-[var(--text-dim)]">
              <ArrowRight className="w-4 h-4 text-amber-400 mt-1 flex-shrink-0" />
              <span><strong className="text-[var(--text)]">Never use &ldquo;will&rdquo; in the unless-clause.</strong> Say <em>&ldquo;Unless it rains&rdquo;</em>, not <em>&ldquo;Unless it will rain.&rdquo;</em> This follows the same rule as all conditional if-clauses.</span>
            </li>
            <li className="flex items-start gap-2 text-[var(--text-dim)]">
              <ArrowRight className="w-4 h-4 text-amber-400 mt-1 flex-shrink-0" />
              <span><strong className="text-[var(--text)]">Clause order is flexible.</strong> You can put the unless-clause first or second: <em>&ldquo;Unless you call, I&apos;ll assume you&apos;re coming&rdquo;</em> or <em>&ldquo;I&apos;ll assume you&apos;re coming unless you call.&rdquo;</em> Use a comma when the unless-clause comes first.</span>
            </li>
            <li className="flex items-start gap-2 text-[var(--text-dim)]">
              <ArrowRight className="w-4 h-4 text-amber-400 mt-1 flex-shrink-0" />
              <span><strong className="text-[var(--text)]">Avoid double negatives.</strong> Since <em>unless</em> already contains a negative meaning, don&apos;t add &ldquo;not&rdquo; inside the unless-clause. Say <em>&ldquo;Unless you try&rdquo;</em>, not <em>&ldquo;Unless you don&apos;t try.&rdquo;</em></span>
            </li>
          </ul>
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">30+ Example Sentences with &ldquo;Unless&rdquo;</h2>
          <p>
          ...
          <h3 className="text-xl font-bold text-[var(--text)] mb-4">Everyday Life</h3>
          <ol className="space-y-2 my-4 pl-5">
            <li className="text-[var(--text-dim)]">Unless you set an alarm, you&apos;ll oversleep.</li>
            <li className="text-[var(--text-dim)]">I won&apos;t buy the jacket unless it goes on sale.</li>
            <li className="text-[var(--text-dim)]">Unless you eat breakfast, you&apos;ll be hungry by 10 a.m.</li>
            <li className="text-[var(--text-dim)]">We can&apos;t leave unless the babysitter arrives.</li>
            <li className="text-[var(--text-dim)]">Unless you tell me what happened, I can&apos;t help you.</li>
            <li className="text-[var(--text-dim)]">She never drinks coffee unless she has a headache.</li>
          </ol>

          <h3 className="text-xl font-bold text-[var(--text)] mb-4">Work &amp; School</h3>
          <ol start={7} className="space-y-2 my-4 pl-5">
            <li className="text-[var(--text-dim)]">Unless you submit the report by Friday, you won&apos;t get feedback in time.</li>
            <li className="text-[var(--text-dim)]">The project will be delayed unless we hire more developers.</li>
            <li className="text-[var(--text-dim)]">Unless the professor cancels class, we have to attend.</li>
            <li className="text-[var(--text-dim)]">You won&apos;t pass the course unless you complete all the assignments.</li>
            <li className="text-[var(--text-dim)]">Unless everyone agrees, we can&apos;t move forward with the plan.</li>
            <li className="text-[var(--text-dim)]">The meeting is still on unless you hear otherwise.</li>
          </ol>

          <h3 className="text-xl font-bold text-[var(--text)] mb-4">Travel &amp; Weather</h3>
          <ol start={13} className="space-y-2 my-4 pl-5">
            <li className="text-[var(--text-dim)]">Unless you book early, flights to Tokyo are expensive.</li>
            <li className="text-[var(--text-dim)]">We&apos;ll go hiking unless the forecast calls for thunderstorms.</li>
            <li className="text-[var(--text-dim)]">Unless you have a visa, you can&apos;t enter the country.</li>
            <li className="text-[var(--text-dim)]">The ferry doesn&apos;t run unless the sea is calm.</li>
            <li className="text-[var(--text-dim)]">Unless you bring sunscreen, you&apos;ll get burned.</li>
            <li className="text-[var(--text-dim)]">I always take an umbrella unless the sky is perfectly clear.</li>
          </ol>

          <h3 className="text-xl font-bold text-[var(--text)] mb-4">Relationships &amp; Feelings</h3>
          <ol start={19} className="space-y-2 my-4 pl-5">
            <li className="text-[var(--text-dim)]">Unless you apologize, she won&apos;t forgive you.</li>
            <li className="text-[var(--text-dim)]">I can&apos;t trust you unless you&apos;re honest with me.</li>
            <li className="text-[var(--text-dim)]">Unless we talk about it, the problem will only get worse.</li>
            <li className="text-[var(--text-dim)]">He won&apos;t change unless he wants to.</li>
            <li className="text-[var(--text-dim)]">Unless you listen, you&apos;ll never understand her point of view.</li>
          </ol>

          <h3 className="text-xl font-bold text-[var(--text)] mb-4">Technology &amp; Science</h3>
          <ol start={24} className="space-y-2 my-4 pl-5">
            <li className="text-[var(--text-dim)]">Unless you update the software, you&apos;ll be vulnerable to security threats.</li>
            <li className="text-[var(--text-dim)]">The experiment fails unless the temperature is controlled precisely.</li>
            <li className="text-[var(--text-dim)]">Unless you back up your data, you risk losing everything.</li>
            <li className="text-[var(--text-dim)]">The phone won&apos;t charge unless you use the original cable.</li>
            <li className="text-[var(--text-dim)]">Unless the code is tested, bugs will appear in production.</li>
          </ol>

          <h3 className="text-xl font-bold text-[var(--text)] mb-4">Formal &amp; Written English</h3>
          <ol start={29} className="space-y-2 my-4 pl-5">
            <li className="text-[var(--text-dim)]">No refund will be issued unless the item is returned within 30 days.</li>
            <li className="text-[var(--text-dim)]">Unless otherwise stated, all prices include tax.</li>
            <li className="text-[var(--text-dim)]">The contract is void unless signed by both parties.</li>
            <li className="text-[var(--text-dim)]">Unless the court rules otherwise, the verdict stands.</li>
            <li className="text-[var(--text-dim)]">Access is denied unless authorized by the administrator.</li>
          </ol>

          <hr className="border-[var(--border)] my-10" />

          {/* SECTION: Common Mistakes */}
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">Common Mistakes with &ldquo;Unless&rdquo;</h2>
          <p>
          ...
          <div className="space-y-4 my-8">
            {[
              ...
              <div key={i} className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6">
                <h3 className="text-lg font-bold text-[var(--text)] m-0 mb-3">{item.mistake}</h3>
                <div className="flex items-start gap-2 mb-2">
                  <X className="w-5 h-5 text-red-400 mt-0.5 flex-shrink-0" />
                ...
                <p className="m-0 text-[var(--text-dim2)] text-sm"><strong>Tip:</strong> {item.tip}</p>
              </div>
            ))}
          </div>
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">&ldquo;Unless&rdquo; in Other Languages</h2>
          <p>
            If you&apos;re learning English from another language &mdash; or learning another language from English &mdash; it helps to see how <strong className="text-[var(--text)]">unless</strong> is translated. Notice that many languages express the same idea with a phrase meaning &ldquo;if&nbsp;&#8230;&nbsp;not&rdquo; rather than having a single dedicated word.
          </p>
          ...
          <div className="grid sm:grid-cols-2 gap-4 my-8">
            {[
              ...
              <div key={i} className="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-5">
                <strong className="text-[var(--text)] block mb-1">{item.lang}</strong>
                <p className="m-0 text-amber-400 font-medium text-sm mb-1">{item.word}</p>
                <p className="m-0 text-sm text-[var(--text-dim2)] italic">{item.note}</p>
              </div>
            ))}
          </div>
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">Learn Grammar Naturally by Watching English Shows with Dual Subtitles</h2>
          <p>
            Grammar rules are important, but they only stick when you encounter them repeatedly in real, meaningful contexts. This is why linguists emphasize <strong className="text-[var(--text)]">comprehensible input</strong> &mdash; hearing and reading the language in natural settings &mdash; as the fastest path to internalization.
          </p>
          <p>
          ...
          <p>
            <strong className="text-[var(--text)]">Dual subtitles</strong> supercharge this process. With your target language on top and your native language below (or vice versa), you can:
          </p>
          <ul className="space-y-2 my-4">
            <li className="flex items-start gap-2 text-[var(--text-dim)]">
              <ArrowRight className="w-4 h-4 text-amber-400 mt-1 flex-shrink-0" />
          ...
          {/* SECTION: FAQ */}
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">Frequently Asked Questions</h2>
          ...
          <div className="space-y-6 my-8">
            {[
              ...
              <div key={i} className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6">
                <h3 className="text-lg font-bold text-[var(--text)] m-0 mb-3">{item.q}</h3>
                <p className="m-0 text-[var(--text-dim)]">{item.a}</p>
              </div>
            ))}
          </div>
          ...
          <div className="bg-[rgba(245,158,11,0.05)] border border-amber-400/20 rounded-2xl p-8 text-center my-12">
            ...
            <h2 className="text-2xl font-bold text-[var(--text)] mb-3">Master English Grammar by Watching, Not Memorizing</h2>
