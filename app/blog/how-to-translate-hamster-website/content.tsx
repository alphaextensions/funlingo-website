    ...
    return (
        <div className="flex flex-col min-h-screen bg-[var(--bg)]">
            ...
            <section className="relative w-full py-16 lg:py-24 px-6 overflow-hidden">
                <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] from-orange-900/40 via-[var(--bg)] to-[var(--bg)] z-0 pointer-events-none" />
            ...
            <main className="w-full max-w-3xl mx-auto px-6 pb-24">
                <div className="prose prose-lg max-w-none text-[var(--text-dim)]">
                    ...
                    {/* Intro */}
                    <h2 className="text-2xl font-bold text-[var(--text)] mb-4">Translate the Hamster Website — Without Losing the Original Text</h2>
                    <p className="leading-relaxed text-[var(--text-dim)] mb-6">
                        The Hamster website — whether you're looking at <strong className="text-[var(--text)]">HammyHome</strong> (the viral virtual hamster simulation), <strong className="text-[var(--text)]">HAMSTER Corporation's</strong> Japanese gaming site, or any other Hamster-related page — is often in a language you don't fully understand. Traditional translators like Google Translate replace the entire page, giving you a rough translation but removing the original text completely.
                    </p>
                    <p className="lead text-xl leading-relaxed text-[var(--text)]">
                    ...
                    {/* Step by step */}
                    <h2 className="text-2xl sm:text-3xl font-bold text-[var(--text)] mb-8">How to Translate the Hamster Website (Step-by-Step)</h2>
                    <p className="text-[var(--text-dim)] mb-6">Translating any Hamster website with Funlingo takes under a minute. Here's how:</p>
                    ...
                    <div className="space-y-6 relative before:absolute before:left-[19px] before:top-4 before:bottom-4 before:w-0.5 before:bg-[var(--surface-2)] mb-16">
                        {[
                            ...
                            <div key={i} className="flex gap-6 relative">
                                <div className="w-10 h-10 rounded-full bg-[#1a1a1a] border border-[#C81FD4]/50 flex items-center justify-center text-[#C81FD4] font-bold shrink-0 z-10">
                                    {i + 1}
                                </div>
                                <div className="pt-2 text-[var(--text-dim)]">
                                    <strong className="text-[var(--text)] block mb-1">{step.title}</strong>
                                    {step.desc}
                                </div>
                            </div>
                        ))}
                    ...
                    <h2 className="text-2xl sm:text-3xl font-bold text-[var(--text)] mb-8">Funlingo Translation Features That Work on the Hamster Website</h2>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-16">
                        <div className="bg-[var(--surface)] p-6 rounded-xl border border-[var(--border)]">
                            ...
                            <h3 className="text-xl font-bold text-[var(--text)] mb-2">Bilingual Side-by-Side Display</h3>
                            <p className="text-sm text-[var(--text-dim)]">See the original Hamster website text alongside your chosen language. Both languages appear together on the same page — no tab switching, no copy-pasting into Google Translate.</p>
                        </div>
                        <div className="bg-[var(--surface)] p-6 rounded-xl border border-[var(--border)]">
                            <MousePointer className="w-8 h-8 text-[#C81FD4] mb-4" />
                            <h3 className="text-xl font-bold text-[var(--text)] mb-2">Word-Level Popup with Native Meaning</h3>
                            <p className="text-sm text-[var(--text-dim)]">Click any word on the Hamster website to see its native meaning in a popup, plus a "View Example" button for contextual usage — a feature unique to Funlingo.</p>
                        </div>
                        <div className="bg-[var(--surface)] p-6 rounded-xl border border-[var(--border)]">
                            <Languages className="w-8 h-8 text-[#C81FD4] mb-4" />
                            <h3 className="text-xl font-bold text-[var(--text)] mb-2">Multi-Translation Selector (1/2/3/All)</h3>
                            <p className="text-sm text-[var(--text-dim)]">Don't settle for one translation. Funlingo lets you choose how many translations to see per word — 1, 2, 3, or All — giving you deeper understanding of each word's meaning.</p>
                        </div>
                        <div className="bg-[var(--surface)] p-6 rounded-xl border border-[var(--border)]">
                            <Zap className="w-8 h-8 text-[#C81FD4] mb-4" />
                            <h3 className="text-xl font-bold text-[var(--text)] mb-2">Pronunciation with Audio Playback</h3>
                            <p className="text-sm text-[var(--text-dim)]">Hear how any word is pronounced with spelled-out cues and audio playback. Especially useful for Japanese sites like hamster.co.jp where reading kanji, hiragana, and katakana can be challenging.</p>
                        </div>
                        <div className="bg-[var(--surface)] p-6 rounded-xl border border-[var(--border)]">
                            <BookOpen className="w-8 h-8 text-[#C81FD4] mb-4" />
                            <h3 className="text-xl font-bold text-[var(--text)] mb-2">Inline Native Meaning for Hard Words</h3>
                            <p className="text-sm text-[var(--text-dim)]">Funlingo automatically shows bracketed native meanings for difficult words directly in the text — so you don't even need to click. You understand tricky vocabulary at a glance.</p>
                        </div>
                        <div className="bg-[var(--surface)] p-6 rounded-xl border border-[var(--border)]">
                            <Layers className="w-8 h-8 text-[#C81FD4] mb-4" />
                            <h3 className="text-xl font-bold text-[var(--text)] mb-2">Save-to-Vocab & Review Deck</h3>
                            <p className="text-sm text-[var(--text-dim)]">Save any word from the Hamster website to your personal vocabulary list. Funlingo includes a built-in review deck so you can revisit and retain what you've learned.</p>
                        </div>
                        <div className="bg-[var(--surface)] p-6 rounded-xl border border-[var(--border)]">
                            <Eye className="w-8 h-8 text-[#C81FD4] mb-4" />
                            <h3 className="text-xl font-bold text-[var(--text)] mb-2">Click-for-Context AI Explanations</h3>
                            <p className="text-sm text-[var(--text-dim)]">When a word or phrase is ambiguous, click for AI-powered explanations that include phrase-level context, grammar notes, and usage examples — not just a dictionary definition.</p>
                        </div>
                        <div className="bg-[var(--surface)] p-6 rounded-xl border border-[var(--border)]">
                            <Check className="w-8 h-8 text-[#C81FD4] mb-4" />
                            <h3 className="text-xl font-bold text-[var(--text)] mb-2">Privacy-First & Lightweight</h3>
                            <p className="text-sm text-[var(--text-dim)]">Funlingo runs as a lightweight Chrome extension with explicit permission controls. Your browsing data stays private — no data selling, no heavy background processes.</p>
                        </div>
                    </div>

                    {/* Which Hamster websites */}
                    <h2 className="text-2xl sm:text-3xl font-bold text-[var(--text)] mb-6">Which Hamster Websites Can You Translate with Funlingo?</h2>
                    <p className="text-[var(--text-dim)] mb-6">
                        Funlingo works on <strong className="text-[var(--text)]">any website</strong> — but here are the most popular Hamster websites people are translating right now:
                    </p>

                    <div className="space-y-6 mb-16">
                        <div className="border-l-4 border-orange-400 pl-6 py-2">
                            <h3 className="text-xl font-semibold text-[var(--text)] mt-0 mb-2">HammyHome (hammyhome.com)</h3>
                            <p className="text-[var(--text-dim)] m-0">
                        ...
                        <div className="border-l-4 border-purple-400 pl-6 py-2">
                            <h3 className="text-xl font-semibold text-[var(--text)] mt-0 mb-2">HAMSTER Corporation (hamster.co.jp)</h3>
                            <p className="text-[var(--text-dim)] m-0">
                        ...
                        <div className="border-l-4 border-blue-400 pl-6 py-2">
                            <h3 className="text-xl font-semibold text-[var(--text)] mt-0 mb-2">Hamster Translation Extension (nodemedia.cn)</h3>
                            <p className="text-[var(--text-dim)] m-0">
                        ...
                        <div className="border-l-4 border-green-400 pl-6 py-2">
                            <h3 className="text-xl font-semibold text-[var(--text)] mt-0 mb-2">Any Other Website</h3>
                            <p className="text-[var(--text-dim)] m-0">
                    ...
                    <div className="bg-gradient-to-br from-[#C81FD4]/20 to-transparent p-8 rounded-xl border border-[#C81FD4]/20 mb-16">
                        <h2 className="text-2xl font-bold text-[var(--text)] mb-6 mt-0">Funlingo vs. Google Translate for the Hamster Website</h2>
                        <p className="text-[var(--text-dim)] mb-6">Google Translate is great for quick comprehension, but Funlingo is built for understanding <em>and</em> learning.</p>
                        ...
                        <div className="space-y-4">
                            {[
                                ...
                                <div key={i} className="flex flex-col md:flex-row gap-4 border-b border-[var(--border)] pb-4 last:border-0">
                                    <div className="flex-1">
                                        <span className="text-[var(--text-dim)] text-xs uppercase tracking-wider block mb-1">{i === 0 ? "Google Translate" : ""}</span>
                                        <span className="text-[var(--text-dim)]">{row.google}</span>
                                    </div>
                                    <div className="flex-1">
                                        <span className="text-[#C81FD4] text-xs uppercase tracking-wider block mb-1">{i === 0 ? "Funlingo" : ""}</span>
                                        <span className="text-[var(--text)] font-medium">{row.funlingo}</span>
                                    </div>
                                </div>
                            ))}
                        </div>
                    ...
                    {/* Use cases */}
                    <h2 className="text-2xl sm:text-3xl font-bold text-[var(--text)] mb-6">How People Use Funlingo on the Hamster Website</h2>
                    <ul className="space-y-4 list-disc pl-5 text-[var(--text-dim)] mb-16">
                        <li><strong className="text-[var(--text)]">Non-English speakers exploring HammyHome:</strong> Understand every button, instruction, and tooltip while learning English vocabulary from a fun, interactive website.</li>
                        <li><strong className="text-[var(--text)]">Gamers reading HAMSTER Corporation news:</strong> Read Japanese game announcements with bilingual display, learning Japanese gaming vocabulary along the way.</li>
                        <li><strong className="text-[var(--text)]">Language learners browsing any foreign site:</strong> Turn daily browsing into language practice — read news, blogs, forums, and documentation in two languages simultaneously.</li>
                        <li><strong className="text-[var(--text)]">Students building vocabulary from real content:</strong> Save unfamiliar words from any website to your vocabulary deck and review them later using Funlingo's built-in review tools.</li>
                    </ul>

                    <hr className="border-[var(--border)] my-12" />

                    {/* FAQ */}
                    <h2 className="text-2xl sm:text-3xl font-bold text-[var(--text)] mb-8">Frequently Asked Questions</h2>
                    <div className="space-y-6 mb-16">
                        {[
                            ...
                            <div key={i} className="border-b border-[var(--border)] pb-6 last:border-0">
                                <h4 className="font-bold text-[var(--text)] text-lg mb-2">Q: {faq.q}</h4>
                                <p className="text-[var(--text-dim)] m-0">A: {faq.a}</p>
                            </div>
                        ))}
                    </div>
                    ...
                    <div className="mt-20 text-center py-12 px-6 rounded-2xl bg-[rgba(200,31,212,0.05)] border border-[#C81FD4]/20">
                        <h2 className="text-3xl font-bold text-[var(--text)] mb-4">Translate the Hamster Website Now — Free</h2>
