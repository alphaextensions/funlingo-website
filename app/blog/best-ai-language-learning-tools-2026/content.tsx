    ...
    <div className="flex flex-col min-h-screen bg-black">
      ...
      <section className="relative w-full py-16 lg:py-24 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto flex flex-col gap-6 text-center lg:text-left">
          <Badge className="w-fit mx-auto lg:mx-0 bg-[rgba(198,66,252,0.1)] text-[#C642FC] border-[#C642FC]/20 px-3 py-1">Language Learning Guide</Badge>
      ...
      <main className="w-full max-w-4xl mx-auto px-6 pb-24">
        <div className="prose prose-invert prose-lg max-w-none text-gray-300">
          ...
          <ul className="space-y-4 list-none pl-0">
            {[
              ...
              <li key={i} className="flex gap-4 p-4 rounded-lg bg-white/5 border border-white/5 hover:border-white/10 transition-colors">
                ...
                <div><strong className="text-white block mb-1">{item.title}</strong>{item.desc}</div>
              </li>
            ))}
          </ul>
          ...
          <div className="my-10 not-prose">
            ...
            <p className="text-xs text-gray-500 mt-2 text-center">Table: Detailed feature comparison of top AI language tools.</p>
          </div>
          ...
          <div className="bg-white/5 rounded-xl p-8 mt-12 border border-white/10">
            ...
            <ul className="space-y-3 list-none pl-0 m-0">
              {["Does it work in your reading/watching flow?","Can it give multiple senses per word?","Does it include quick audio playback?","Can you save & review?","Does it respect privacy & permissions?"].map((item, i) => (
                <li key={i} className="flex items-center gap-3"><input type="checkbox" readOnly checked className="accent-[#C642FC] h-5 w-5" /><span className="text-gray-200">{item}</span></li>
              ))}
            </ul>
          </div>
