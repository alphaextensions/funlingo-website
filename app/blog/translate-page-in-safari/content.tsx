    ...
    <div className="flex flex-col min-h-screen bg-[var(--bg)]">
      ...
      <section className="relative w-full py-16 lg:py-24 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto flex flex-col gap-6 text-center lg:text-left">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-[var(--text)] leading-tight">How to Translate a Web Page in Safari (Mac, iPhone, iPad)</h1>
      ...
      <main className="w-full max-w-3xl mx-auto px-6 pb-24">
        <div className="prose prose-lg max-w-none text-[var(--text-dim)]">
          ...
          <p>If you are interested in using web translation as part of a language learning strategy, check out our guide on <a href="/blog/website-translation-language-learning" className="text-[#C81FD4] hover:underline">website translation for language learning</a>.</p>

          <hr className="border-[var(--border)] my-10" />

          {/* Mac Instructions */}
          <div className="flex items-center gap-3 mb-4">
            <Monitor className="w-6 h-6 text-[#C81FD4]" />
            <h2 className="text-3xl font-bold text-[var(--text)] m-0">Translate a Web Page in Safari on Mac</h2>
          </div>
            ...
            <ol className="space-y-4 list-none pl-0 m-0 counter-reset-none">
              {[
                ...
                <li key={i} className="flex gap-4">
                  <span className="flex-shrink-0 w-8 h-8 rounded-full bg-[#C81FD4]/20 text-[#C81FD4] flex items-center justify-center text-sm font-bold">{item.step}</span>
                  <div>
                    <strong className="text-[var(--text)]">{item.title}</strong>
                    <p className="text-[var(--text-dim)] text-sm mt-1 mb-0">{item.desc}</p>
                  </div>
                </li>
              ))}
            </ol>
          ...
          <p><strong className="text-[var(--text)]">Requirement:</strong> macOS Big Sur (11.0) or later. If you are running an older version of macOS, you will not have the built-in translation feature and will need a third-party solution.</p>

          <hr className="border-[var(--border)] my-10" />

          {/* iPhone Instructions */}
          <div className="flex items-center gap-3 mb-4">
            <Smartphone className="w-6 h-6 text-[#C81FD4]" />
            <h2 className="text-3xl font-bold text-[var(--text)] m-0">Translate a Web Page in Safari on iPhone</h2>
          </div>
          ...
          <p><strong className="text-[var(--text)]">Requirement:</strong> iOS 14 or later. Most iPhones in use today support this, but if your device is stuck on an older version, the translate option will not appear.</p>

          <hr className="border-[var(--border)] my-10" />

          {/* iPad Instructions */}
          <div className="flex items-center gap-3 mb-4">
            <Tablet className="w-6 h-6 text-[#C81FD4]" />
            <h2 className="text-3xl font-bold text-[var(--text)] m-0">Translate a Web Page in Safari on iPad</h2>
          </div>
          ...
          <p><strong className="text-[var(--text)]">Requirement:</strong> iPadOS 14 or later.</p>

          <hr className="border-[var(--border)] my-10" />

          {/* Supported Languages */}
          <h2 className="text-3xl font-bold text-[var(--text)]">Supported Languages</h2>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
              {[
                ...
                <div key={i} className="flex items-center gap-2 text-sm text-[var(--text-dim)]">
                  <Globe className="w-3 h-3 text-[#C81FD4] shrink-0" />
                  {lang}
                </div>
              ))}
            </div>
          ...
          {/* Limitations */}
          <h2 className="text-3xl font-bold text-[var(--text)]">Limitations of Safari Translation</h2>
          ...
          <div className="space-y-3 my-8">
            {[
              ...
              <div key={i} className="flex gap-3 bg-[var(--surface)] rounded-lg p-4 border border-[var(--border)]">
                <AlertCircle className="w-5 h-5 text-yellow-400 mt-0.5 shrink-0" />
                <div>
                  <strong className="text-[var(--text)] text-sm">{item.title}</strong>
                  <p className="text-[var(--text-dim)] text-sm mt-1 mb-0">{item.desc}</p>
                </div>
              </div>
            ))}
          ...
          <h2 className="text-3xl font-bold text-[var(--text)]">What to Do When Translation Is Not Available</h2>
          ...
          <ul className="space-y-2 list-none pl-0 my-6">
            {[
              ...
              <li key={i} className="flex items-start gap-3 text-sm">
                ...
                <span className="text-[var(--text-dim)]">{item}</span>
              </li>
            ))}
          </ul>
          ...
          <h2 className="text-3xl font-bold text-[var(--text)]">Third-Party Alternatives</h2>
          ...
          <div className="space-y-4 my-8">
            ...
            <div className="bg-[var(--surface)] rounded-xl p-6 border border-[var(--border)]">
              <h3 className="text-xl font-bold text-[var(--text)] mt-0 mb-2"><a href="https://www.microsoft.com/en-us/translator/" target="_blank" rel="noopener noreferrer" className="text-[var(--text)] hover:text-[#C81FD4] transition-colors">Microsoft Translator</a></h3>
              <p className="text-[var(--text-dim)] mb-3">Microsoft offers a Translator extension for Safari on Mac. It adds a translate button to your toolbar and supports over 70 languages. The translation quality is comparable to Google Translate for major languages. It is free to use and integrates smoothly with Safari&apos;s interface.</p>
              <div className="flex gap-4 text-sm">
          ...
          <h2 className="text-3xl font-bold text-[var(--text)]">Using Web Translation for Language Learning</h2>
            ...
            <ul className="space-y-3 list-none pl-0 m-0">
              {[
                ...
                <li key={i} className="text-sm">
                  <strong className="text-[var(--text)]">{item.title}:</strong>
                  <span className="text-[var(--text-dim)] ml-1">{item.desc}</span>
                </li>
              ))}
            </ul>
          </div>
          ...
          <h2 className="text-3xl font-bold text-[var(--text)]">Funlingo: A Better Option for Language Learners</h2>
          <div className="bg-[rgba(200,31,212,0.05)] rounded-xl p-8 border border-[#C81FD4]/20 my-8">
            <p className="text-[var(--text-dim)] mb-4">If your goal is not just to understand a page but to actually learn the language, a standard translator falls short. Funlingo takes a different approach &mdash; instead of replacing text, it helps you learn from the content you are already watching and reading online.</p>
