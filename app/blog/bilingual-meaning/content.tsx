    ...
    <div className="flex flex-col min-h-screen bg-[var(--bg)]">
      ...
      <section className="relative w-full py-16 lg:py-24 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto flex flex-col gap-6 text-center lg:text-left">
          <Badge className="w-fit mx-auto lg:mx-0 bg-[rgba(200,31,212,0.1)] text-[#C81FD4] border-[#C81FD4]/20 px-3 py-1">Dictionary &amp; Education</Badge>
      ...
      <main className="w-full max-w-3xl mx-auto px-6 pb-24">
        <div className="prose prose-lg max-w-none text-[var(--text-dim)]">
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">The Simple Definition of Bilingual</h2>
          <p>
            At its most basic level, <strong className="text-[var(--text)]">bilingual</strong> comes from the Latin roots <em>bi-</em> (meaning &ldquo;two&rdquo;) and <em>lingua</em> (meaning &ldquo;language&rdquo; or &ldquo;tongue&rdquo;). A bilingual person is someone who uses two languages in their daily life, whether at home, at work, in social settings, or across all of these contexts.
          </p>
          <p>
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">The Linguistic Definition: Going Deeper</h2>
          <p>
          ...
          <p>
            This means a person who reads academic papers in English but speaks Mandarin at home is bilingual. A heritage speaker who understands their grandparents&apos; Tagalog but responds in English is bilingual. The key criterion is <strong className="text-[var(--text)]">functional use</strong>, not flawless symmetry.
          </p>

          <hr className="border-[var(--border)] my-10" />

          {/* SECTION: Types of Bilingualism */}
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">Types of Bilingualism</h2>
          <p>
            ...
            {[
              ...
              <div key={i} className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6">
                <div className="flex items-center gap-3 mb-3">
                  <div className="text-[#C81FD4]">{item.icon}</div>
                  <h3 className="text-xl font-bold text-[var(--text)] m-0">{item.title}</h3>
                </div>
                <p className="m-0 text-[var(--text-dim)]">{item.desc}</p>
              </div>
            ))}
          </div>

          <p>
            There are also distinctions like <strong className="text-[var(--text)]">receptive bilingualism</strong> (understanding a language without being able to speak it fluently) and <strong className="text-[var(--text)]">productive bilingualism</strong> (being able to both understand and produce speech in both languages). Heritage speakers often fall into the receptive category for their family&apos;s language.
          </p>

          <hr className="border-[var(--border)] my-10" />

          {/* SECTION: Examples */}
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">Examples of Bilingual Countries and Communities</h2>
          <p>
            ...
            {[
              ...
              <div key={i} className="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-5">
                <div className="flex items-center gap-2 mb-2">
                  <Globe className="w-4 h-4 text-[#C81FD4]" />
                  <strong className="text-[var(--text)]">{item.country}</strong>
                </div>
                <p className="m-0 text-sm text-[var(--text-dim)]">{item.detail}</p>
              </div>
            ))}
          </div>
          ...
          {/* SECTION: Benefits */}
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">Benefits of Being Bilingual</h2>
          <p>
            ...
            {[
              ...
              <div key={i} className="bg-[var(--surface)] border border-[var(--border)] rounded-xl p-6">
                <div className="flex items-center gap-3 mb-4">
                  <div className="text-[#C81FD4]">{section.icon}</div>
                  <h3 className="text-lg font-bold text-[var(--text)] m-0">{section.title}</h3>
                </div>
                <ul className="space-y-2 m-0 pl-0 list-none">
                  {section.points.map((point, j) => (
                    <li key={j} className="flex items-start gap-2 text-[var(--text-dim)]">
                      <ArrowRight className="w-4 h-4 text-[#C81FD4] mt-1 flex-shrink-0" />
                      <span>{point}</span>
                    </li>
                  ))}
                </ul>
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">How to Become Bilingual as an Adult</h2>
          <p>
          ...
          <div className="space-y-4 my-8">
            {[
              ...
              <div key={i} className="flex gap-4 p-5 rounded-xl bg-[var(--surface)] border border-[var(--border)]">
                <div className="flex-shrink-0 w-10 h-10 rounded-full bg-[rgba(200,31,212,0.15)] flex items-center justify-center text-[#C81FD4] font-bold text-lg">{item.step}</div>
                <div>
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mb-6">The Role of Immersion Tools: How Funlingo Helps Build Bilingual Comprehension</h2>
          <p>
          ...
          <div className="space-y-4 my-8">
            {[
              ...
              <div key={i} className="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-5">
                <strong className="text-[var(--text)] block mb-2">{item.title}</strong>
                <p className="m-0 text-[var(--text-dim)] text-base">{item.desc}</p>
              </div>
            ))}
          </div>
          ...
          {/* CTA */}
          <div className="bg-[rgba(200,31,212,0.05)] border border-[#C81FD4]/20 rounded-2xl p-8 text-center my-12">
            <Lightbulb className="w-10 h-10 text-[#C81FD4] mx-auto mb-4" />
            <h2 className="text-2xl font-bold text-[var(--text)] mb-3">Start Your Bilingual Journey Today</h2>
