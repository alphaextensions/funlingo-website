    ...
    <div className="flex flex-col min-h-screen bg-[var(--bg)]">
      ...
      <section className="relative w-full py-16 lg:py-24 px-6 overflow-hidden">
        ...
        <div className="relative z-10 max-w-4xl mx-auto flex flex-col gap-6 text-center lg:text-left">
          ...
          <h1 className="text-4xl sm:text-5xl lg:text-6xl font-bold text-[var(--text)] leading-tight">Best Netflix Language Learning Extensions (2026): 5 Tools Tested & Compared</h1>
      ...
      <main className="w-full max-w-3xl mx-auto px-6 pb-24">
        <div className="prose prose-lg max-w-none text-[var(--text-dim)]">
          ...
          <p>We tested and compared the five most popular Netflix language learning extensions available in 2026. Below you will find detailed reviews, a side-by-side comparison table, and guidance on choosing the right tool for your learning style. If you are also interested in how <a href="/blog/best-dual-subtitle-extension" className="text-[#C81FD4] hover:underline">dual subtitle extensions</a> work across platforms, or want broader tips on <a href="/blog/netflix-language-learning" className="text-[#C81FD4] hover:underline">learning languages with Netflix</a>, check out our dedicated guides.</p>

          <p className="text-[var(--text-dim)] mb-6">Before we dive into the extensions, make sure you understand how to set up <a href="/blog/netflix-dual-subtitles" className="text-[#C81FD4] hover:underline">dual subtitles on Netflix</a> — it takes under 30 seconds.</p>

          <hr className="border-[var(--border)] my-10" />

          {/* Extension 1: Funlingo */}
          <div className="bg-[var(--surface)] rounded-xl p-8 border border-[#C81FD4]/30 my-8 relative overflow-hidden">
            <div className="absolute top-0 right-0 bg-[#C81FD4] text-[var(--text)] text-xs font-bold px-4 py-1 rounded-bl-lg flex items-center gap-1"><Crown className="w-3 h-3" /> Best Free Option</div>
            <h2 className="text-3xl font-bold text-[var(--text)] mb-2 mt-0">1. Funlingo</h2>
            <p className="text-[#C81FD4] font-medium mb-4">Free &mdash; No account required</p>
            ...
            <h3 className="text-lg font-bold text-[var(--text)] mb-3 mt-6">Key Features</h3>
              ...
              <div>
                ...
                <ul className="space-y-1 list-none pl-0 m-0">
                  {["Completely free", "Multi-translation selector is unique", "Works across 3 major platforms", "No account needed to start", "Lightweight and fast"].map((p, i) => (
                    <li key={i} className="flex items-center gap-2 text-sm text-[var(--text-dim)]"><Check className="w-3 h-3 text-green-400 shrink-0" />{p}</li>
                  ))}
                </ul>
              </div>
              <div>
                ...
                <ul className="space-y-1 list-none pl-0 m-0">
                  {["Chrome only (no Firefox or Safari)", "Newer extension with smaller community", "No built-in flashcard SRS system yet"].map((c, i) => (
                    <li key={i} className="flex items-center gap-2 text-sm text-[var(--text-dim)]"><X className="w-3 h-3 text-red-400 shrink-0" />{c}</li>
                  ))}
                </ul>
              </div>
            </div>
          ...
          <div className="bg-[var(--surface)] rounded-xl p-8 border border-[var(--border)] my-8">
            <h2 className="text-3xl font-bold text-[var(--text)] mb-2 mt-0">2. <a href="https://www.languagereactor.com" target="_blank" rel="noopener noreferrer" className="text-[var(--text)] hover:text-[#C81FD4] transition-colors">Language Reactor</a></h2>
            ...
            <ul className="space-y-2 list-none pl-0 mb-6">
              {[
                ...
                <li key={i} className="flex items-start gap-3 text-sm"><Check className="w-4 h-4 text-[var(--text-dim)] mt-1 shrink-0" /><span className="text-[var(--text-dim)]">{f}</span></li>
              ))}
            </ul>
            ...
            <p className="text-[var(--text-dim)] mt-6 mb-0">See our full <a href="/blog/language-reactor-alternatives" className="text-[#C81FD4] hover:underline">Language Reactor alternatives guide</a> for more options.</p>
          </div>

          {/* Extension 3: Trancy */}
          <div className="bg-[var(--surface)] rounded-xl p-8 border border-[var(--border)] my-8">
            <h2 className="text-3xl font-bold text-[var(--text)] mb-2 mt-0">3. <a href="https://www.trancy.org" target="_blank" rel="noopener noreferrer" className="text-[var(--text)] hover:text-[#C81FD4] transition-colors">Trancy</a></h2>
            ...
            <p className="text-[var(--text-dim)] mt-6 mb-0">For a detailed look at Trancy&apos;s AI conversation feature, see our <a href="/blog/aitalk-review" className="text-[#C81FD4] hover:underline">AiTalk review</a>.</p>
          </div>

          {/* Extension 4: Immersive Translate */}
          <div className="bg-[var(--surface)] rounded-xl p-8 border border-[var(--border)] my-8">
            <h2 className="text-3xl font-bold text-[var(--text)] mb-2 mt-0">4. <a href="https://immersivetranslate.com" target="_blank" rel="noopener noreferrer" className="text-[var(--text)] hover:text-[#C81FD4] transition-colors">Immersive Translate</a></h2>
          ...
          {/* Extension 5: eJOY */}
          <div className="bg-[var(--surface)] rounded-xl p-8 border border-[var(--border)] my-8">
            <h2 className="text-3xl font-bold text-[var(--text)] mb-2 mt-0">5. <a href="https://ejoy-english.com" target="_blank" rel="noopener noreferrer" className="text-[var(--text)] hover:text-[#C81FD4] transition-colors">eJOY</a> English</h2>
          ...
          {/* Comparison Table */}
          <h2 className="text-3xl font-bold text-[var(--text)] mt-16 mb-6">Side-by-Side Comparison Table</h2>
                ...
                <table className="w-full min-w-[700px] text-left border-collapse" aria-label="Netflix language learning extensions comparison">
                  <thead className="bg-[var(--surface)]">
                    <tr className="border-b border-[var(--border)]">
                      <th className="p-4 text-[var(--text-dim)] font-medium text-sm">Feature</th>
                      <th className="p-4 text-[var(--text)] font-bold text-sm bg-[rgba(200,31,212,0.15)] border-t-2 border-t-[#C81FD4]">Funlingo</th>
                      <th className="p-4 text-[var(--text-dim)] font-semibold text-sm">Language Reactor</th>
                      <th className="p-4 text-[var(--text-dim)] font-semibold text-sm">Trancy</th>
                      <th className="p-4 text-[var(--text-dim)] font-semibold text-sm">Immersive Translate</th>
                      <th className="p-4 text-[var(--text-dim)] font-semibold text-sm">eJOY</th>
                    </tr>
                  </thead>
                  <tbody className="divide-y divide-white/5 text-sm">
                    {[
                      ...
                      <tr key={i} className="group hover:bg-[var(--surface)] transition-colors">
                        <td className="p-4 text-[var(--text-dim)] font-medium">{row.feature}</td>
                        {row.values.map((val, j) => (
                          <td key={j} className={`p-4 ${j === 0 ? "bg-[rgba(200,31,212,0.05)] text-[var(--text)]" : "text-[var(--text-dim)]"}`}>
                            <span className={row.highlights[j] ? "text-[#C81FD4] font-semibold" : ""}>{val}</span>
                          </td>
                        ))}
                      </tr>
                    ))}
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mt-16 mb-6">How to Choose the Right Extension</h2>
          ...
          <div className="space-y-4 my-8">
            {[
              ...
              <div key={i} className="flex gap-4 p-5 rounded-lg bg-[var(--surface)] border border-[var(--border)]">
                <div className="text-[#C81FD4] mt-1 shrink-0">{item.icon}</div>
                <div>
                  <strong className="text-[var(--text)] block mb-1">{item.scenario}</strong>
                  <span className="text-[var(--text-dim)] text-sm">{item.pick}</span>
                </div>
              </div>
            ))}
          </div>

          <div className="bg-[rgba(200,31,212,0.05)] border border-[#C81FD4]/20 rounded-xl p-8 my-8">
            <h3 className="text-xl font-bold text-[var(--text)] mb-4 mt-0">Why we recommend starting with Funlingo</h3>
            ...
            <p className="text-[var(--text-dim)] m-0">The multi-translation selector &mdash; letting you choose to see 1, 2, 3, or all meanings for any word &mdash; is a feature unique to Funlingo and especially valuable for languages where words carry multiple meanings depending on context.</p>
          </div>

          <hr className="border-[var(--border)] my-12" />

          <h2 className="text-2xl sm:text-3xl font-bold text-[var(--text)] mb-6">How to Choose a Language Learning Chrome Extension for Netflix</h2>
          <p className="text-[var(--text-dim)] mb-6">Choosing the right Chrome extension for Netflix language learning comes down to three questions. First, how much are you willing to pay? Funlingo is the only extension that offers full dual subtitles, word-level translation, pronunciation audio, and vocabulary saving completely free. Language Reactor and Trancy lock their best features behind $6-10/month subscriptions.</p>
          ...
          <h2 className="text-3xl font-bold text-[var(--text)] mt-16 mb-8">Frequently Asked Questions</h2>
          <div className="space-y-6">
            ...
            <div>
              <strong className="text-[var(--text)] block mb-1">Does Netflix have a built-in language learning feature?</strong>
              <p className="text-[var(--text-dim)] text-sm">No. Netflix doesn&apos;t have built-in language learning tools — it only offers standard subtitles in one language at a time. To get dual subtitles, word lookups, vocabulary saving, or pronunciation guides, you need a Chrome extension. Funlingo adds all of these features for free.</p>
            </div>
            <div>
              <strong className="text-[var(--text)] block mb-1">Can I use Netflix language learning extensions on my phone or TV?</strong>
              <p className="text-[var(--text-dim)] text-sm">Netflix language learning extensions are Chrome browser add-ons and only work on desktop computers. They can&apos;t be installed on the Netflix mobile app, smart TVs, or tablets. To use dual subtitles on a bigger screen, watch Netflix in Chrome on your laptop and connect it to your TV via HDMI or Chromecast.</p>
            </div>
          </div>
